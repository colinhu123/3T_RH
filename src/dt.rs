...
pub fn get_local_dt(state1: State, dx: f64, dy: f64) -> f64 {
    ...
    let s1 = ((A * OMEGA_EI * CVE.powi(4)
        ...
        + OMEGA_ER * CVE.powi(4) * CVI * state1.rho)
        .powi(2)
        - 4.0
            * A
            * OMEGA_EI
            ...
            * CVE.powi(4)
            * CVI
            * (4.0 * A * ee.powi(3) + CVE.powi(4) * state1.rho + CVE.powi(3) * CVI * state1.rho))
        .sqrt();

    let s2 = -(A * OMEGA_EI * CVE.powi(4)
        + A * OMEGA_EI * CVE.powi(3) * CVI
        + 4.0 * A * OMEGA_ER * CVI * ee.powi(3)
        + OMEGA_ER * CVE.powi(4) * CVI * state1.rho);
    ...
    let dj = (KAPPA_E / (CVE * state1.rho))
        .max(KAPPA_I / (CVI * state1.rho))
        .max(KAPPA_R / A);
